\expandafter\ifx\csname ver@layout.sty\endcsname\relax
  \@latex@warning@no@line{Package `layout' not loaded. Command \string\layoutpicture{} will have no effect.}
...
\newcommand\lay@layoutpic{%
  ...
  \begin{picture}(\cnt@paperwidth,\cnt@paperheight)
    ...
    \put(\ref@margin,\ref@head){%
      \begingroup\color{gray}\framebox(\cnt@textwidth,\cnt@headheight)%
    ...
    \put(\ref@margin,\ref@foot){%
      \begingroup\color{gray}\framebox(\cnt@textwidth,\fheight){\tiny\sffamily\Footertext}\endgroup}
    \put(\ref@marginpar,\ref@body){%
      \begingroup\color{gray}\framebox(\cnt@marginparwidth,\cnt@textheight)%
  ...
  \begingroup\footnotesize\ttfamily
    \begin{tabular}{@{}rl@{\hspace{20pt}}rl}
      ...
      5 & \Show{cnt}{headheight} & 
      6 & \Show{cnt}{headsep} \\
      7 & \Show{cnt}{textheight} & 
      8 & \Show{cnt}{textwidth} \\
      9 & \Show{cnt}{marginparsep}&
      10& \Show{cnt}{marginparwidth} \\
      11& \Show{cnt}{footskip}   &   
        & \Show{cnt}{marginparpush} \rlap{(\notshown)}\\
        & \Show{ref}{hoffset}    &   
        & \Show{ref}{voffset} \\
        & \Show{cnt}{paperwidth} &   
