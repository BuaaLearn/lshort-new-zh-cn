...
\begin{figure}[!p]
\caption{本文档的页面参数示意图（偶数页；由 \pkg{layout} 宏包生成）。} \label{fig:layouts}
\centering
\layoutpicture*
\end{figure}
...
\begin{description}
  \item[\cls{article} 文档类，\texttt{twoside} 选项：] 偶数页页眉为页码和节标题，奇数页页眉为小节标题和页码；
  \item[\cls{book/report} 文档类，\texttt{twoside} 选项：] 偶数页页眉为页码和章标题，奇数页页眉为节标题和页码；
...
在双面排版、headings / myheadings 页眉页脚样式下，\Arg{left-mark} 和 \Arg{right-mark} 的内容分别预期出现在左页（偶数页）和右页（奇数页）。
...
\footnote{但是这不能改变页眉页脚的斜体样式（\cmd{slshape}），斜体是定义在 headings 样式里的。
如果不喜欢斜体，办法之一是在 \cmd{mark\-both} 等命令的参数里先使用 \cmd{normal\-font} 再使用想要的字体样式命令，
