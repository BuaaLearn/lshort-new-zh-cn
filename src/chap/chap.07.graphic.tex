...
\begin{itemize}
  ...
  \item \textbf{\hologo{METAPOST} \& Asymptote} \par
...
\footnote{\texttt{latex + dvipdfmx} 编译方式要在 \pkg{tikz} 宏包之前加载 \pkg{graphicx} 宏包并指定 \texttt{dvipdfmx} 选项。}：
\begin{command}
...
连续使用连线时，可以使用 \texttt{cycle} 作为最后一个点，生成闭合的路径。
\begin{example}
\begin{tikzpicture}
\draw (0,0) -- (1,1) 
   -- (2,0) -- cycle;
...
\begin{itemize}
  ...
  \item \texttt{above / below / left / right / above left / \ldots} \par
  \texttt{anchor=\Arg{position}} 的等效写法，\texttt{above} 相当于 \texttt{anchor=south}，以此类推。
...
\end{tikzpicture}
\end{Verbatim}
\caption{\TikZ\ 绘图示例源代码。}\label{code:tikz-example}
\end{sourcecode}
...
\node[above right,outer sep=0.2cm, rounded corners,
  ...
  at (b) {$\displaystyle \int_a^b {f(x)\,\mathrm{d}x} = F(b) - F(a)$};
\end{tikzpicture}
\caption{\TikZ\ 绘图示例。}\label{fig:tikz-example}
\end{figure}
\endinput
