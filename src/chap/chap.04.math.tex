...
或者用 \amscmd{notag} 命令取消为公式编号（\LaTeX\ 提供了与之基本等效的命令 \cmd{nonumber}）。
...
以及带各种定界符的矩阵 \amsenv{pmatrix}$($、\amsenv{bmatrix}$[$、\amsenv{Bmatrix}$\{$、\amsenv{vmatrix}$\vert$、\amsenv{Vmatrix}$\Vert$。
使用这些环境时，无需给定列格式：
\begin{example}
\[
\begin{matrix}
