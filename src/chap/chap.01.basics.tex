...
\begin{flushleft}
\begin{description}
\item[\texttt{10pt}, \texttt{11pt}, \texttt{12pt}] \quad 指定文档的基本字号。缺省为 \texttt{10pt}。

\item[\texttt{a4paper}, \texttt{letterpaper}, \ldots] \quad 指定纸张大小，默认为美式纸张 \texttt{letterpaper}。
...
\item[\texttt{titlepage}, \texttt{notitlepage}] 指定标题命令 \cmd{maketitle} 是否单独成页。
\cls{article} 缺省为 \texttt{notitlepage}，\cls{report} 和 \cls{book} 缺省为 \texttt{titlepage}。

\item[\texttt{onecolumn}, \texttt{twocolumn}] \quad 指定单栏/双栏排版。
